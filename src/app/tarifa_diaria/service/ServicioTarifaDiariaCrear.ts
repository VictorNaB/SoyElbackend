...
class ServicioTarifaDiariaCrear {
    protected static async CrearTarifaDiaria(obj: TarifaDiaria, res: Response): Promise<any> {
        await pool.task(async(consulta)=>{
            ...
            if(existeTarifa.cantidad == 0){
                caso=2;
                objCreado= await consulta.one(SQL_TARIFA_DIARIA.ADD,[obj.codParqueadero,obj.codTipoVehiculo,obj.valorTarifaDiaria]);
            }
